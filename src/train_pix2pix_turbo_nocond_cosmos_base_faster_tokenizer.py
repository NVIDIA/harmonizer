...
from utils.training_utils import PairedDatasetV2, parse_args_paired_training
...
def main(args):
    ...
    if args.resume is not None:
        if os.path.isdir(args.resume):
            ...
            ckpt_files = sorted(
                ckpt_files, key=lambda x: int(x.split("/")[-1].replace("model_", "").replace(".pkl", ""))
            )
            print("=" * 50)
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                lpips_val = net_lpips(x_tgt_pred, x_tgt)
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.viz_freq == 1:
                        log_dict = {
                            ...
                            "train/model_output": [
                                wandb.Image(x_tgt_pred[idx].clamp(-1,1).float().detach().cpu(), caption=f"idx={idx}")
                                for idx in range(B)
                            ],
                        }
                        for k in log_dict:
                    ...
                    if args.eval_freq > 0 and global_step % args.eval_freq == 1:
                        ...
                        for step, batch_val in enumerate(dl_val):
                            ...
                            with torch.no_grad():
                                ...
                                caption_tokens = clip.tokenize(batch_val["caption"], truncate=True).to(
                                    x_tgt_pred.device
                                )
                                clipsim, _ = net_clip(x_tgt_pred_renorm, caption_tokens)
